...
from chatterbug.domain.model import DEFAULT_WHISPER_MODEL, TranscriptionPreset, EngineKind
...
from chatterbug.cli.helpers import build_sink
...
app = typer.Typer(
    help="🐛 ChatterBug - Local-first speech transcription",
...
def transcribe(
    ...
    EXAMPLES:
      ...
      chatterbug transcribe audio.mp3 --output transcript.txt
      chatterbug transcribe podcast.flac --engine voxtral_local
      chatterbug transcribe meeting.m4a --preset fast
...
def check() -> None:
    ...
    [white]Run this after installing ChatterBug to ensure everything works.[/white]
    """
...
def check_vllm(
    ...
    The server must be started separately with [cyan]chatterbug serve-vllm[/cyan].
    """
    config = load_config()
...
def serve_vllm(
    ...
    [bold]Example:[/bold]
      [cyan]chatterbug serve-vllm --model openai/whisper-large-v3-turbo[/cyan]
